    ...
    function getinstalledPackages($types = array('module', 'langpack')){
    	...
    	foreach($installeds as $installed)
		{
			...
			switch($type)
			{
				...
				$packages[] = array('name' => $name, 'version' => $version, 'type' => $type, 'published_date' => $date_entered, 'description' => $description, 'uninstallable' =>$uninstallable, 'file_install' => urlencode( $filename ), 'file' => urlencode( $file_uninstall ), 'enabled' => $enabled_string);
				break;
				default:
				break;
			}
