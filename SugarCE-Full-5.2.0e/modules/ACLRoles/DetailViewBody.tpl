...
{foreach from=$ACTION_NAMES item="ACTION_NAME" }
	<td class="tabDetailViewDL" style="text-align: center;"><b>{$ACTION_NAME}</b></td>
{foreachelse}
		...
		{foreach from=$TYPES item="ACTIONS" key="TYPE_NAME"}
			{foreach from=$ACTIONS item="ACTION" key="ACTION_NAME_ACTIVE"}
				{if $ACTION_NAME==$ACTION_NAME_ACTIVE}
					...
					<td  class="tabDetailViewDF" width='{$TDWIDTH}%' align='center'><div align='center' class="acl{$ACTION.accessLabel}"><b>{$ACTION.accessName}</b></div></td>
				{/if}
			{/foreach}
		{/foreach}
		{if $ACTION_FIND=='false'}
			<td nowrap class="tabDetailViewDF" width='{$TDWIDTH}%' style="text-align: center;">
