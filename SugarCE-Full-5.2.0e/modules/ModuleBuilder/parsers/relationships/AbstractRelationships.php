...
class AbstractRelationships
{
    ...
    static function findRelatableModules ($includeActivitiesSubmodules = true)
    {
        ...
        $relatableModules [ 'Activities' ] [ 'default' ] = $GLOBALS [ 'mod_strings' ] [ 'LBL_DEFAULT' ] ;
    ...
    protected function saveLabels ($basepath , $installDefPrefix , $relationshipName , $labelDefinitions)
    {
        ...
        // remove any old label files first - can't do this in the later loop as may have multiple labels / module so we need to append
        $headerString = "<?php\n" ;
        foreach ( $labelDefinitions as $definition )
        {
            ...
            {
                $fh = fopen ( $filename, 'w' ) ;
                fputs ( $fh, $headerString, strlen ( $headerString ) ) ;
                fclose ( $fh ) ;
            }
        }
        ...
        foreach ( $labelDefinitions as $definition )
        {
            ...
            if ($definition['module'] == 'application') {
                $labelString = override_value_to_string_recursive2('app_list_strings', 
                                    $definition [ 'system_label' ], $definition [ 'display_label' ]);
            } else {
                $labelString = '$mod_strings [ "' . $definition [ 'system_label' ] . '" ] = "' . $definition [ 'display_label' ] . "\" ;\n" ;
            }
            
            $fh = fopen ( $filename, 'a' ) ;
            fputs ( $fh, $labelString, strlen ( $labelString ) ) ;
            fclose ( $fh ) ;
