...
class DeployedSubpanelImplementation extends AbstractMetaDataImplementation implements MetaDataImplementationInterface
{
    ...
    function __construct ($subpanelName , $moduleName)
    {
        ...
        $this->_originalSubPanelDefs = $spd->layout_defs ;
        ...
        if (! empty ( $spd->layout_defs ))
            if (array_key_exists ( strtolower ( $subpanelName ), $spd->layout_defs [ 'subpanel_setup' ] ))
            {
                ...
                $this->_originalDefs = $originalSubpanel->get_list_fields ();
                ...
                if (! empty ( $subPanelParentModuleName ) && isset ( $beanListLower [ strtolower ( $subPanelParentModuleName ) ] ))
                {
                    ...
                    foreach ( $subPanelParentModule->field_defs as $key => $def )
                    {
                        $key = strtolower ( $key ) ;
                        if (! isset ( $def [ 'vname' ] ))
                            $def [ 'vname' ] = (isset ( $def [ 'label' ] )) ? $def [ 'label' ] : $def [ 'name' ] ;
                            // remove all of the fields that aren't allowed in the viewdefs
                        
//                      if ((empty ( $def [ 'source' ] ) || $def [ 'source' ] == 'db' || $def [ 'source' ] == 'custom_fields') && (isset ( $def [ 'type' ] ) && $def [ 'type' ] != 'id') && strcmp ( $key, 'deleted' ) != 0 || (isset ( $def [ 'name' ] ) && substr ( $def [ 'name' ], - 5 ) === '_name') || ! empty ( $def [ 'custom_type' ] ) && (empty ( $def [ 'dbType' ] ) || $def [ 'dbType' ] != 'id') && (empty ( $def [ 'dbtype' ] ) || $def [ 'dbtype' ] != 'id') || (! empty ( $def [ 'studio' ] ) && $def [ 'studio' ] == 'visible'))
