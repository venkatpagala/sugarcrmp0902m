...
class ParserModifyListView extends ModuleBuilderParser
{
	...
	function getAvailableFields ()
	{
		...
		foreach ( $modFields as $key => $def )
		{
            ...
            if (((empty ( $def [ 'source' ] ) || $def [ 'source' ] == 'db' || $def [ 'source' ] == 'custom_fields') && $def [ 'type' ] != 'id' && (empty ( $def [ 'dbType' ] ) || $def [ 'dbType' ] != 'id')  && strcmp ( $key, 'deleted' ) != 0) || (isset ( $def [ 'name' ] ) && strpos ( $def [ 'name' ], "_name" ) !== false) || (! empty ( $def [ 'studio' ] ) && $def [ 'studio' ] == 'visible'))
				{
