...
function createTreePanel(treeData, params) {
	...
	var tree = new Ext.tree.TreePanel(params);
...
function addChildNodes(parentNode, parentData) {
	var nodes = parentData.nodes;
	for (i in nodes) {
		if (typeof(nodes[i]) == 'object') {
			var node = new Ext.tree.TreeNode({
				href : 		nodes[i].data.href,
				action : 	nodes[i].data.action,
				id: 		nodes[i].data.id,
				text: 		nodes[i].data.label,
				expanded: 	nodes[i].custom.expanded
			})
