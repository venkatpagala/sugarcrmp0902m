    ...
    function getDashlets($category='') {
        ...
        foreach($dashletsFiles as $className => $files) {
            if(!empty($files['meta']) && is_file($files['meta'])) {
				...
				// generate icon
                if (!empty($dashletMeta[$files['class']]['icon']) && sugar_is_file('custom/'. $dashletMeta[$files['class']]['icon'])){
                	$icon = '<img src="custom/' . $dashletMeta[$files['class']]['icon'] .'" border="0" align="absmiddle" />';
                } else if (!empty($dashletMeta[$files['class']]['icon']) && sugar_is_file($dashletMeta[$files['class']]['icon'])){
                	$icon = '<img src="' . $dashletMeta[$files['class']]['icon'] .'" border="0" align="absmiddle" />';
                }
                else{
