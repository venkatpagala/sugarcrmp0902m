		...
		switch (command) {
			...
			case "mceLink":
				...
				if (inst.settings['insertlink_callback']) {
					...
					tinyMCE.openWindow(template, {href : href, target : target, title : title, onclick : onclick, action : action, className : style_class, inline : "yes", editor_id : inst.editorId});
				}
			...
			case "mceImage":
				...
				if (inst.settings['insertimage_callback']) {
					...
					tinyMCE.openWindow(template, {src : src, alt : alt, border : border, hspace : hspace, vspace : vspace, width : width, height : height, align : align, title : title, onmouseover : onmouseover, onmouseout : onmouseout, action : action, inline : "yes", editor_id : inst.editorId});
