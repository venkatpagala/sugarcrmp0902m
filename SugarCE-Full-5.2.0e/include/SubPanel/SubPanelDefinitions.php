	...
	function get_available_tabs ($FromGetModuleSubpanels=false)
	{
		...
		if (isset ( $this->layout_defs [ 'subpanel_setup' ] )) // bug 17434 - belts-and-braces - check that we have some subpanels first
		{
			foreach ( $this->layout_defs [ 'subpanel_setup' ] as $key => $values_array )
			{
                ...
                if (array_key_exists ( $values_array [ 'module' ], $modules_exempt_from_availability_check ) or array_key_exists ( $values_array [ 'module' ], $modListHeader ) && (! ACLController::moduleSupportsACL ( $values_array [ 'module' ] ) || ACLController::checkAccess ( $values_array [ 'module' ], 'list', true )))				
				{
