...
class SugarBean
{
    ...
    function SugarBean()
    {
    	...
    	if((false == $this->disable_vardefs && empty($loaded_defs[$this->object_name])) || !empty($GLOBALS['reload_vardefs']))
    	{
    		...
    		if(isset($GLOBALS['dictionary'][$this->object_name]))
    		{
    ...
    function get_importable_fields()
    {
        ...
        if (!empty($fieldDefs)) {
            foreach ($fieldDefs as $key=>$value_array) {
                if ( (isset($value_array['importable']) && $value_array['importable'] == 'false')
...
*/
	    ...
	    foreach ( $this->field_defs as $def )
	    {
           if ($def [ 'type' ] == 'relate' && isset ( $def [ 'id_name'] ) && isset ( $def [ 'link'] ) && isset ( $def[ 'save' ]) )
	       {
	           ...
	           if (isset( $this->field_defs[ $def [ 'link' ] ] ))
	           {
                    ...
                    //bug 26970, calling save twice in a row will produce an error as the link will be created twice. 
                    //We need to ensure that the link is a string meaning that we have not saved and converted into a link object.
		...
		if(!empty($new_rel_id)){
			if($new_rel_relname == 'Emails' && !$this->new_with_id) {
				return;

			} else if($this->load_relationship($new_rel_link)){
		...
		foreach($subpanel_list as $this_subpanel)
		{
			...
			if(!$this_subpanel->isDatasourceFunction() or ($this_subpanel->isDatasourceFunction() and isset($this_subpanel->_instance_properties['generate_select']) and $this_subpanel->_instance_properties['generate_select']==true))
			{
				...
				if ($this_subpanel->isDatasourceFunction()) {
					...
					$query_array = $parentbean->$related_field_name->getQuery(true,array(),0,'',true, null, true);
				}
	...
	*/
    ...
    /**
     * Return the list query used by the list views and export button. Next generation of create_list_query function.
     *
    	...
    	foreach($fields as $field=>$value)
    	{
       		...
       		if($data['type'] == 'relate' && isset($data['link']))
    		{
    			...
    			if(!empty($this->$data['link']))
    			{
    				...
    				//jchi fix #25652
    				if ($this->table_name != $join_table_name) {
	...
	*/
	function fill_in_additional_detail_fields()
	{
		...
		if(!empty($this->field_defs['modified_user_id']))
	...
	function fill_in_additional_parent_fields() {
		...
		if(!empty($this->parent_type) and isset($GLOBALS['beanFiles'][$GLOBALS['beanList'][$this->parent_type]])) {
			require_once($GLOBALS['beanFiles'][$GLOBALS['beanList'][$this->parent_type]]);
			$parentBean = new $GLOBALS['beanList'][$this->parent_type]();

			// We need to set the $mod_strings to the parent_type value for the retrieve call on SugarBean
			global $mod_strings, $current_language;
			// save a copy of the mod_strings - note that these may not be the mod_strings for this module, for example during MergeRecords
			...
			// reset the mod_strings
			$mod_strings = $tmp_mod_strings ;
		...
		foreach($this->field_defs as $field=>$value){
			if(isset($this->$field)){
				...
				if(isset($sensitiveFields[$field]))
					continue;
                                if(!isset($cache[$field]))$cache[$field] = strtoupper($field);
				...
				//#26481 , add radioenum type in if condition
