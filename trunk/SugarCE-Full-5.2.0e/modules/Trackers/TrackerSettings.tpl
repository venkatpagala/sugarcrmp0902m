...
<td class="dataLabel" width="50%">{$mod.LBL_ENABLE}</td>
</tr>
{foreach name=trackerEntries from=$trackerEntries key=key item=entry}
<tr>
<td class="dataLabel" width="50%">{$entry.label}:&nbsp;{sugar_help text=$entry.helpLabel}</td>
<td class="dataField" width="50%"><input type="checkbox" id="{$key}" name="{$key}" value="1" {if !$entry.disabled}CHECKED{/if}>
</tr>
{/foreach}
<tr>
...
<tr>
<td class="dataLabel" width="20%">{$mod.LBL_TRACKER_PRUNE_INTERVAL}</td>
