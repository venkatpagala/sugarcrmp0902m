...
class AbstractRelationship
{
    ...
    function __construct ($definition)
    {
        ...
        foreach ( array ( 'readonly' , 'deleted' , 'relationship_only', 'for_activities', 'is_custom' ) as $key )
    ...
    function buildLabels ()
    {
        ...
        if (!$this->relationship_only)
        {
        	$labelDefinitions [] = array (
        		...
        		'system_label' => 'LBL_' . strtoupper ( $this->relationship_name . '_FROM_' . $this->lhs_module ) . '_TITLE' ,
        		'display_label' => empty($this->lhs_label) ? translate ( $this->lhs_module ) : $this->lhs_label,
        	) ;
            $labelDefinitions [] = array (
            	'module' => $this->lhs_module ,
            	'system_label' => 'LBL_' . strtoupper ( $this->relationship_name . '_FROM_' . $this->rhs_module ) . '_TITLE' ,
     ...
     */
    protected function getSubpanelDefinition ($relationshipName , $sourceModule , $subpanelName)
    {
        ...
        /*
         * 20751: add the MultiSelect parameter to the select button
         *
         * MultiSelect is enabled by adding 'mode'=>'MultiSelect' to the array definition of the select button in the top_button array definition
         * The subpanel definition is loaded by SubPanelDefinitions->open_layout_defs(); first from modules/{module}/metadata/subpaneldefs.php and then overidden from custom/modules/{module}/Ext/Layoutdefs/layoutdefs.ext.php
         * Anything under $layout_defs[{module}]["subpanel_setup"][{subpanelname}] will be included in the definition used
         *
         * However, it would be unreliable to attempt to override the definition of the select button only; as the top_button definition is not an associative array we can't specify which element in the definition to override
         * Unfortunately therefore we must instead override the entire top_button array with a new definition that includes MultiSelect.
         * In an attempt to minimize differences between the original and our overidden layout, we base the new layout on the current top_button layout for the subpanel
         * This gets a bit problematic though for ModuleBuilder modules as we base the layout on the subpanel on the development system, which may not look at all like the layout on the system we're deploying on...
         *
         * TODO: make this optional on a module by module basis
         * TODO: implement a decent override mechanism in ModuleLoader for merging in changes such as MultiSelect to an existing layout
         */

        // load in the top_buttons from the subpanel definition
        // note that we can't use the methods in SubPanelDefinitions for this as they work off the current module, and we haven't defined this subpanel yet for this module
		if (file_exists ( "modules/{$sourceModule}/metadata/subpanels/{$subpanelName}.php" ))
		{
			require "modules/{$sourceModule}/metadata/subpanels/{$subpanelName}.php" ;

			foreach ( $subpanel_layout [ 'top_buttons' ] as $key => $button )
			{
				if ( $button [ 'widget_class' ] == 'SubPanelTopSelectButton')
					$subpanel_layout [ 'top_buttons' ] [ $key ] [ 'mode'] = 'MultiSelect' ;
			}

			$subpanelDefinition [ 'top_buttons' ] = $subpanel_layout [ 'top_buttons' ] ;
		}

        /*
         * 20751: end of changes
         */
     ...
     * @param string $relationshipName  Name of the relationship
     */
    protected function getLinkFieldDefinition ($sourceModule , $relationshipName, $right_side = false)
    {
        $vardef = array ( ) ;
     ...
     * @param string $relationshipName  Name of the relationship
     */
    protected function getLink2FieldDefinition ($sourceModule , $relationshipName, $right_side = false)
    {
        $vardef = array ( ) ;
     ...
     */
    protected function getRelateFieldDefinition ($sourceModule , $relationshipName )
    {
        $vardef = array ( ) ;
    ...
    static function parseRelationshipType ($type)
    {
        ...
        foreach ( array ( MB_ONETOONE , MB_ONETOMANY , MB_MANYTOMANY ) as $canonicalType )
        {
