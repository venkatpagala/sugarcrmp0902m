...
if($focus->campaign_type == 'NewsLetter'){
    echo "\n<p>\n";
    echo get_module_title($mod_strings['LBL_MODULE_NAME'], $mod_strings['LBL_NEWSLETTER_WIZARD_START_TITLE'].$focus->name, true);
    echo "\n</p>\n";
}else{
    echo "\n<p>\n";
    echo get_module_title($mod_strings['LBL_MODULE_NAME'], $mod_strings['LBL_CAMPAIGN_WIZARD_START_TITLE'].$focus->name, true);
    echo "\n</p>\n";
}
...
$image_path = $theme_path."images/";
    ...
    $ss->assign("CAMPAIGN_TBL", create_campaign_summary ($focus,$image_path));
    $ss->assign("TARGETS_TBL", create_target_summary ($focus,$image_path));
    $ss->assign("TRACKERS_TBL", create_tracker_summary ($focus));
    if($focus->campaign_type =='NewsLetter' || $focus->campaign_type =='Email'){
        $ss->assign("MARKETING_TBL", create_marketing_summary ($focus,$image_path));
    }
...
*/
    ...
    //account for use within wizards
        echo "\n<p>\n";
        echo get_module_title($mod_strings['LBL_MODULE_NAME'], $mod_strings['LBL_CAMPAIGN_WIZARD'].$focus->name, true);
        echo "\n</p>\n";
...
function create_campaign_summary  ($focus,$image_path){
    ...
    foreach($fields as $key){
                if(!empty($focus->$key)){
                    ...
                    $cmpgn_tbl .= "<tr><td class='dataLabel' width='15%'>".$mod_strings[$focus->field_name_map[$key]['vname']]."</td><td class='dataField'>".$focus->$key."</td></tr>";
...
function create_marketing_summary  ($focus,$image_path){
    ...
    $mrkt_tbl .= "<tr class='tabDetailView'><td colspan='3'><h4> ".$mod_strings['LBL_WIZ_MARKETING_TITLE']." </h4></td>" .
            ...
            foreach($mrkt_lists as $mrkt_id){
             ...
             if( $colorclass== "class='evenListRowS1' bgColor='$even_bg' "){
              ...
              if(isset($mrkt_focus->name) && !empty($mrkt_focus->name)){
                ...
                $mrkt_tbl  .= "<td $colorclass scope='row' width='25%'>".$mrkt_focus->from_name."</td>";
                $mrkt_tbl  .= "<td $colorclass  scope='row' width='15%'>".$mrkt_focus->status."</td>";
...
function create_target_summary  ($focus,$image_path){
    global $mod_strings,$app_strings,$app_list_strings, $odd_bg, $even_bg, $hilite_bg;
    $colorclass = '';
            ...
            foreach($pl_lists as $pl_id){
             ...
             if( $colorclass== "class='evenListRowS1' bgColor='$even_bg' "){
              ...
              if(isset($pl_focus->id) && !empty($pl_focus->id)){
                ...
                $pl_tbl  .= "<td class='$colorclass' scope='row' width='50%'><a class='listViewTdLinkS1' href='index.php?action=DetailView&module=ProspectLists&return_module=Campaigns&return_action=WizardHome&return_id=" .$focus->id. "&record=".$pl_focus->id."'>";
                $pl_tbl  .=  $pl_focus->name."</a></td>";
                $pl_tbl  .= "<td $colorclass scope='row' width='30%'>$type</td>";
                $pl_tbl  .= "<td $colorclass scope='row' width='15%'>".$pl_focus->get_entry_count()."</td>";
                $pl_tbl  .= "<td $colorclass scope='row' width='5%' align='right'><a class='listViewTdLinkS1' href='index.php?action=EditView&module=ProspectLists&return_module=Campaigns&return_action=WizardHome&return_id=" .$focus->id. "&record=".$pl_focus->id."'>";
                $pl_tbl  .= "<img src='".$image_path."edit_inline.gif' border=0></a>&nbsp;";
                $pl_tbl  .= "<a class='listViewTdLinkS1' href='index.php?action=DetailView&module=ProspectLists&return_module=Campaigns&return_action=WizardHome&return_id=" .$focus->id. "&record=".$pl_focus->id."'>";
    ...
    if(count($trkr_lists)>0){
        ...
        foreach($trkr_lists as $trkr_id){
          ...
          if(isset($ct_focus->tracker_name) && !empty($ct_focus->tracker_name)){
            ...
            $trkr_tbl  .= "<td $colorclass scope='row' ><a class='listViewTdLinkS1' href='index.php?action=DetailView&module=CampaignTrackers&return_module=Campaigns&return_action=WizardHome&return_id=" .$focus->id. "&record=".$ct_focus->id."'>";
            $trkr_tbl  .= $ct_focus->tracker_name."</a></td>";
            $trkr_tbl  .= "<td $colorclass scope='row' width='15%'>".$ct_focus->tracker_url."</td>";
...
function create_wiz_menu_items($type,$mrkt_string,$camp_url,$summ_url){
    ...
    if(isset($steps)  && !empty($steps)){
        ...
        foreach($steps as $name=>$step){
            $nav_html .= "<tr><td class='tabDetailViewDL2' nowrap><div id='nav_step$i'><a href='".$camp_url.$i."'>$name</a></div></td></tr>";
            $i=$i+1;
        }
    }
