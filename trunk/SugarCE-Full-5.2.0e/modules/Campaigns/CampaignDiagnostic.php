...
$mboxTable = "<table border ='0' width='100%'  class='tabDetailView' cellpadding='0' cellspacing='0'>";
...
if(isset($mbox) && count($mbox)>0){
    $mboxTable .= "<tr><td colspan='5' style='text-align: left;'><b>" .count($mbox) ." ". $mod_strings['LBL_MAILBOX_CHECK1_GOOD']." </b>.</td></tr>";
        $mboxTable .= "<tr><td width='20%' class='tabDetailViewDF'><b>".$mod_strings['LBL_MAILBOX_NAME']."</b></td>"
                   ...
                   .  " <td width='20%' class='tabDetailViewDF'><b>".$mod_strings['LBL_LIST_STATUS']."</b></td></tr>";

    foreach($mbox as $details){
        $mboxTable .= "<tr><td>".$details['name']."</td>";
        $mboxTable .= "<td class='tabDetailViewDL2'>".$details['email_user']."</td>";
        $mboxTable .= "<td class='tabDetailViewDL2'>".$details['mailbox']."</td>";
        $mboxTable .= "<td class='tabDetailViewDL2'>".$details['server_url']."</td>";
        $mboxTable .= "<td class='tabDetailViewDL2'>".$details['status']."</td></tr>";
    }

}else{
    //if array is empty, then set "bad" message and increment health counter
    $mboxTable .=  "<tr><td colspan='5' class='tabDetailViewDF'><b class='error'>". $mod_strings['LBL_MAILBOX_CHECK1_BAD']."</b></td></tr>";
...
if (strstr($focus->settings['notify_fromaddress'], 'example.com')){
    ...
    if($focus->settings['mail_sendtype']=='SMTP'){
     $conf_msg .= " <td width='20%' class='tabDetailViewDF'><b>".$mod_strings['LBL_MAIL_SMTPSERVER']."</b></td>" 
        ...
        $conf_msg .= "<tr class='tabDetailViewDL2'><td>".$focus->settings['notify_fromname']."</td>";
...
foreach ($scheds as $funct){
  if( ($funct['job']==$check_sched1)  ||   ($funct['job']==$check_sched2)){
        $sched_mes = 'use';
        $sched_mes_body .= "<tr><td class='tabDetailViewDL2' style='text-align: left;'>".$funct['name']."</td>";
        $sched_mes_body .= "<td class='tabDetailViewDL2' style='text-align: left;'>".$funct['status']."</td></tr>";
 ...
 * $total Parameter is the total number things being checked.
 * $num parameter is the "health" parameter being tracked whenever there is something wrong.  (higher number =bad)
 * $image_path is simply the image path
 * */

function define_image($num, $total, $image_path='themes/default/images/'){
    //if health number is equal to total number then all checks failed, set red image
    if($num == $total){
        //red
        return "<img src='".$image_path."red_camp.gif' align='absmiddle'> ";
    }elseif($num == 0){
        //if health number is zero, then all checks passed, set green image
        //green
       return "<img src='".$image_path."green_camp.gif' align='absmiddle' > ";
    }else{
        //if health number is between total and num params, then some checks failed but not all, set yellow image
        //yellow
        return "<img src='".$image_path."yellow_camp.gif' align='absmiddle' > ";
    }
}
