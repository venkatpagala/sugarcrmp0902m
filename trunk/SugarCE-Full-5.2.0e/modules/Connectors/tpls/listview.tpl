...
<tr height='20'>
<td width="5%" class="listViewThS1">{$APP.LBL_SELECT_BUTTON_LABEL}</td>
...
{foreach name=rowIteration from=$DATA key=id item=bean}
    ...
    <tr height='20' onmouseover="setPointer(this, '{$rowData.id}', 'over', '{$_bgColor}', '{$bgHilite}', '');" onmouseout="setPointer(this, '{$rowData.ID}', 'out', '{$_bgColor}', '{$bgHilite}', '');" onmousedown="setPointer(this, '{$rowData.id}', 'click', '{$_bgColor}', '{$bgHilite}', '');">
		...
		<span id='adspan_{$bean->id}' onmouseout="return clear_source_details()" onmouseover="get_source_details('{$source_id}', '{$bean->id}', 'adspan_{$bean->id}')" onmouseout="return nd(1000);"><img border='0' src='themes/Sugar/images/MoreDetail.png' width='8' height='7'></span>
		</td>
		{foreach from=$displayColumns key=colHeader item=params}
		{if $colHeader != 'id'}
