...
SUGAR.subpanelUtils = function() {
	...
	return {
		...
		inlineSave: function(theForm, subpanel) {
            ...
            if(subpanel == 'activities' && document.getElementById(theForm).status[document.getElementById(theForm).status.selectedIndex].value == 'Held') {
                reloadpage = true;
            }
		...
		sendAndRetrieve: function(theForm, theDiv, loadingStr, subpanel) {
			function success(data) {
				...
				if(isIE||isSafari) SUGAR.util.evalScript(data.responseText);
