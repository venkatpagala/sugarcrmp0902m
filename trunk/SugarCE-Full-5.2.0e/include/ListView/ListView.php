...
function getUserVariable($localVarName, $varName) {
	...
	function processUnionBeans($sugarbean, $subpanel_def) {
        ...
        $response_query = '';
        if(isset($response['query']))$response_query = $response['query']; 
		...
		global $list_view_row_count;
		$list_view_row_count = $row_count;
		$this->processListNavigation('dyn_list_view', 'CELL', $current_offset, $next_offset, $previous_offset, $row_count, $sugarbean,$subpanel_def, $response_query);
	 ...
	 * Contributor(s): ______________________________________..
	 */
	 function processListNavigation($xtemplateSection, $html_varName, $current_offset, $next_offset, $previous_offset, $row_count, $sugarbean=null, $subpanel_def=null, $response_query='') {
	...
	*/
			...
			$selected_objects_span = "&nbsp;|&nbsp;{$this->local_app_strings['LBL_LISTVIEW_SELECTED_OBJECTS']}<input class='listViewPaginationTdS1' style='border: 0px; background: transparent; font-size: inherit; color: inherit' type='text' readonly name='selectCount[]' value='" . (!empty($select_entire_list) ? $row_count : 0) . "' />";
			...
			if($this->show_paging == true) {
				...
				if ($subpanel_def != null) {
                    ...
                    //attempt to get the query to recreate this subpanel if it was not passed in
                    ...
                    if (!empty($response_query)){        
                        $html_text .= $subpanelTiles->get_buttons($subpanel_def, $response_query);
                    }else{
	...
	function getArrowEnd($image_path) {
		...
		return ".$ext' width='$width' height='$height' align='absmiddle' alt=".translate('LBL_SORT').">";
	}
