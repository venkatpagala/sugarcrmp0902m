...
class FreeTDSHelper extends MssqlHelper 
{
        ...
        )
    {
        ...
        if(($type == 'char' || $type == 'text')) {
        	if(!empty($table) && !preg_match('/(^workflow_.*?)|(^data)|(expressions)|(systems)|(^timeperiods$)|(^tracker_queries$)|(^upgrade_history$)|(^worksheet$)|(^relationships$)/is', $table)) {
                return $map[$type];
        	} else {
                return $map['n'.$type];     	
            }
        } else if($type == 'varchar' && !empty($name) && preg_match('/($id^)|(.*?_id$)|(.*?_id_del$)/is', $name)) {
        	return 'varchar';
        }
